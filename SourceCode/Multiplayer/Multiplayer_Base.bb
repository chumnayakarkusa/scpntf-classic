...
Function LoadMultiplayerMenuResources()
	...
	mp_I\TickIcon = LoadImage("GFX\menu\tick.png");
	...
	mp_I\ConnectionIcons = LoadAnimImage("GFX\menu\connection_icons.png",30,30,0,5)
...
Function MPMainLoop()
	...
	While (ft\accumulator>0.0)
			...
			Select mp_I\Gamemode\ID
				Case Gamemode_Waves
					If (mp_I\Gamemode\Phase Mod 2)=1 Lor InLobby() Then
						ShouldPlay = 37
					Else
						If ShouldPlay < 35 Lor ShouldPlay > 36 Then
							ShouldPlay = 36-Rand(0,1)
						EndIf	
					EndIf
		...
		PlayGunSoundsMP()
		AnimateGunsServer()
		If Players[mp_I\PlayerID]\CurrHP > 0 And (Not ConsoleOpen) And (Not mp_I\ChatOpen) And (Not MenuOpen) And (Not InLobby()) And (Not IsModerationOpen()) And (Not IsInVote()) Then
									...
									For g = Each Guns
										If g\ID = Players[TempID]\WeaponInSlot[Players[TempID]\SelectedSlot] Then
											If (g\GunType <> GUNTYPE_MELEE And g\GunType <> GUNTYPE_SHOTGUN) Then
												If Players[TempID]\Ammo[Players[TempID]\SelectedSlot] > ammo And (Players[TempID]\Ammo[Players[TempID]\SelectedSlot] < g\MaxCurrAmmo Lor ammo > 0) Then
...
;							EndIf
						...
						Players[i]\Ammo[SLOT_PRIMARY] = Steam_PullShort()
						Players[i]\Ammo[SLOT_SECONDARY] = Steam_PullShort()
						Players[i]\ReloadAmmo[SLOT_PRIMARY] = Steam_PullByte()
						Players[i]\ReloadAmmo[SLOT_SECONDARY] = Steam_PullByte()
...
;							EndIf
						...
						Steam_PushShort(Players[j]\Ammo[SLOT_PRIMARY])
						Steam_PushShort(Players[j]\Ammo[SLOT_SECONDARY])
						Steam_PushByte(Players[j]\ReloadAmmo[SLOT_PRIMARY])
						Steam_PushByte(Players[j]\ReloadAmmo[SLOT_SECONDARY])
				...
				For n.NPCs = Each NPCs
					If ShouldSyncNPC(n) Then
						...
						Steam_PushFloat(n\State)
						Steam_PushFloat(n\State2)
						Steam_PushFloat(n\State3)
						Steam_PushInt(n\HP)
